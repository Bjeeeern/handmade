				...
				EntityIndex++, Entity++)
		{
			...
			if(Step == (Steps-1))
			{
				...
				if(GameState->DEBUG_VisualiseMinkowskiSum)
				{
					...
					entity Vehicle = GetEntityByLowIndex(Entities, Player->RidingVehicle);

					if(Entity->Collides)
					{
						if(Vehicle)
						{
							if(EntityIndex != VehicleIndex &&
				...
				{
					if(Entity->HighIndex == GameState->DEBUG_CollisionLoopEntityIndex) 
					{
						DrawFrame(Buffer, RectCenterDim(EntityPixelPos, CollisionMarkerPixelDim), 
