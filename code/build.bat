...
ml64 /Fo intrinsics.obj %asmflags% "%source%\intrinsics.asm"

cl %defines% %errors% %flags% -Fmwin32_game.map "%source%\win32_game.cpp" -link %linkflags% %links% -PDB:win32_game.pdb

cl %defines% %errors% %flags% -Fm "%source%\%filename%.cpp" -LD -link %linkflags% intrinsics.obj -EXPORT:GameGetSoundSamples -EXPORT:GameUpdate -PDB:game_%random%.pdb -OUT:game.dll
